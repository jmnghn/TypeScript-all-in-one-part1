...
함께 읽어 볼 라이브러리들의 '코드'가 아니라 '타입'을 보는 것이 목적이다.<br />

### 기본 지식

- Main rule: 타입스크립트는 최종적으로 자바스크립트로 변환된다.(⭐️) 순수한 타입스크립트 코드를 돌릴 수 있는 것은 [deno](https://deno.land/)지만, 대중화가 되지는 않았다. <br />
  브라우저와 Node.js는 모두 자바스크립트파일을 실행하고 이를 통해 웹 애플리케이션이 동작한다.
- 타입스크립트는 언어이자 컴파일러(tsc, TypeScript Complier)이다. 컴파일러는 타입스크립트 코드를 자바스크립트로 바꿔준다.
- tsc는 (`tsc --init`커맨드로도 생성할 수 있는) `tsconfig.json`에 따라 타입스크립트 코드를 자바스크립트로 바꿔준다.<br />
  input인 `.ts`와 output인 `.js` 모두에 영향을 끼치므로 `tsconfig.json` 설정을 반드시 봐야한다.
...
`--noEmit` 옵션을 사용하지 않을 경우, 타입검사에 error는 발생하지만, 자바스크립트 변환파일을 생성한다.
